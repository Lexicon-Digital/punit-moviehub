    ...
    public ChatGptQuery(string schema, string message)
    {
        Messages.Add(new Message
        {
            ...
            Content = $"Given that I have an SQLite3 Database with the following schema:\n\n ${schema} \n\n" +
                      ...
                      $"query against the database. IT'S VERY IMPORTANT THAT YOU ONLY GIVE ME THE QUERY WITHOUT ANY ADDITIONAL TEXT, COMMENTS OR FORMATTING," +
                      $"BECAUSE YOUR QUERY RESPONSE WILL DIRECTLY BE RUN ON THE DATABASE. Please be mindful that the question should only return SELECT " +
                      $"queries, and no UPDATE or DELETE queries. Any questions related to updating or deleting anything in the database " +
                      $"should be rejected: \"{message}\""
        });
    }
